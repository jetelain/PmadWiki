...
public class WikiAdminController_EditAccessControlTests : WikiAdminControllerTestBase
{
    ...
    [Fact]
    public async Task EditAccessControl_Get_GroupWithNullLabel_UsesNameAsLabel()
    {
        // Arrange
        _options.UsePageLevelPermissions = true;
        ...
        Assert.Equal("group1", model.Groups[0].Name);
        Assert.Equal("group1", model.Groups[0].Label);
    }

    [Fact]
    public async Task EditAccessControl_Get_GroupWithEmptyLabel_UsesNameAsLabel()
    {
        // Arrange
        _options.UsePageLevelPermissions = true;
...
* | users | editors",
    ...
    public async Task EditAccessControl_Post_CallsGetWikiUserWithRequireUserTrue()
    {
        ...
        // Assert
        _mockUserService.Verify(
            x => x.GetWikiUserAsync(It.IsAny<ClaimsPrincipal>(), true, It.IsAny<CancellationToken>()),
            Times.Once);
    }
