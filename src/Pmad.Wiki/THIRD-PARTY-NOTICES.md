# Third-Party Notices

This file contains notices for third-party software included with or used by Pmad.Wiki.

## Mergely

**License:** Mozilla Public License Version 1.1 (MPL-1.1)  
**Copyright:** © Jamie Peabody  
**Website:** https://www.mergely.com/  
**Used for:** Visual side-by-side diff comparison of wiki page revisions

Mergely is licensed under the Mozilla Public License Version 1.1. The complete license text can be found in `src/Pmad.Wiki/wwwroot/lib/mergely/LICENSE`.

### Mergely Third-Party Dependencies

Mergely itself utilizes:
- **CodeMirror** - Licensed under MIT License

---

## Markdig

**License:** BSD 2-Clause License  
**Copyright:** © 2016-present, Alexandre Mutel  
**Website:** https://github.com/xoofx/markdig  
**Used for:** Markdown parsing and rendering

---

## Bootstrap

**License:** MIT License  
**Copyright:** © 2011-2024 The Bootstrap Authors  
**Website:** https://getbootstrap.com/  
**Used for:** UI framework and styling

---

## Bootstrap Icons

**License:** MIT License  
**Copyright:** © 2019-2024 The Bootstrap Authors  
**Website:** https://icons.getbootstrap.com/  
**Used for:** Icon library

---

## Reveal.js

**License:** MIT License
**Copyright:** © 2011-2024 Hakim El Hattab, http://hakim.se, and reveal.js contributors
**Website:** https://revealjs.com/
**Used for:** Slide show presentation framework

---

## License Summaries

### Mozilla Public License Version 1.1 (MPL-1.1)

The Mozilla Public License is a free and open-source software license. Key points:
- Source code modifications must be made available under MPL
- Can be combined with proprietary code
- Patent grant included
- Weak copyleft (file-level)

Full license text: https://www.mozilla.org/en-US/MPL/1.1/

### MIT License

A permissive free software license that permits reuse with minimal restrictions. Key points:
- Commercial use permitted
- Modification permitted
- Distribution permitted
- Private use permitted
- Provides limited liability and warranty disclaimers

---

## Additional Information

For questions about licensing or third-party software usage, please refer to the individual license files in the project or contact the project maintainers.

Last updated: 2026-03-09
